...
**Executive summary of Hoffman et al. (2026) revisions.** This revision treats the C-PSD slope convention, transport-induced slope structure, sampling depth, polymer density, and water-sediment PSD coherence as explicit uncertainty sources rather than leaving them implicit. The most important correction is that the Koelmans/Coffin correction factor integrates the **differential** PSD slope (*a*~psd~ = *a*~cpsd~ − 1), not the cumulative C-PSD slope. Numerical before/after values are reported in Section 5.1 and the Hoffman changelog; the expected directional effect is a larger count-based exposure correction while preserving the qualitative contrast that Food Dilution is the risk-dominant ERM and Tissue Translocation remains much lower.
